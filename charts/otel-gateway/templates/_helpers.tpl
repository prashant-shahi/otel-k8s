...
helm.sh/chart: {{ include "otel-gateway.chart" . }}
...
Create env
*/}}
{{- define "otel-gateway.env" -}}
{{/*
    ====== GENERATED ENVIRONMENT VARIABLES ======
*/}}
{{- $genEnv := dict -}}
{{- $_ := set $genEnv "OTEL_COMPONENT" "otel-gateway" -}}
{{- $_ := set $genEnv "OTEL_SERVICE_NAME" "otel-gateway" -}}
{{- $_ := set $genEnv "OTEL_RESOURCE_ATTRIBUTES" "otel.component=$(OTEL_COMPONENT),k8s.pod.uid=$(K8S_POD_UID),k8s.pod.ip=$(K8S_POD_IP)" -}}
{{/*
...
*/}}
{{- $prefix := (include "otel-gateway.fullname" .) }}
{{- $secretEnv := dict -}}
...
{{- template "otel-gateway.renderEnv" $completeEnv -}}
